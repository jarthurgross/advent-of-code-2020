...
runInstruction (mask, mem) (Mem addr val) = (mask, foldr (\addr' mem' -> Map.insert addr' val mem') mem $ applyMask mask addr)

applyMask :: String -> Int -> [Int]
applyMask mask = (map bitStringToInt) . expandFloatingBitstring . (applyStringMask mask) . intToBitstring
...
applyStringMask = zipWith (\m b -> if m == '0' then b else m)

expandFloatingBitstring :: String -> [String]
expandFloatingBitstring = (foldr (\x xs -> (:) <$> x <*> xs) [""]) . (map (\x -> if x == 'X' then ['0', '1'] else [x]))
